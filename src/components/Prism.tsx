...
export function Prism({
  ...
  useEffect(() => {
    ...
    const theme1 = hexToVec3(color1)
    const theme2 = hexToVec3(color2)
    ...
    const init = () => {
      ...
      try {
        ...
        initAttempts += 1
        if (initAttempts < 4 && !destroyed) {
          requestAnimationFrame(init)
          return
        }
        container.classList.add('prism-fallback-active')
        return
      }

      container.classList.remove('prism-fallback-active')
    ...
    const fragment = /* glsl */ `
      ...
      void main(){
        ...
        if (uUseThemeColors > 0.5) {
          float energy = clamp(length(col) * 1.35, 0.0, 1.0);
          float blend = clamp(L + sin(iTime * uTimeScale + p.y * 2.0) * 0.08, 0.0, 1.0);
          col = mix(uColor1, uColor2, blend) * energy;
        }
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uTimeScale: { value: TS },
        uColor1: { value: theme1 },
        uColor2: { value: theme2 },
        uUseThemeColors: { value: 1 },
      },
    })
  ...
  return <div ref={containerRef} className={`prism-container ${className}`.trim()} />
}
